      SUBROUTINE DGEQPB( JOB, M, N, K, A, LDA, C, LDC, JPVT, IRCOND,
...
*
         ...
         ELSE
            NB = MAX( ILAENV( INB, 'DGEQRF', ' ', M, N, 0, 0 ),
     $                ILAENV( INB, 'DORMQR', ' ', M, N, 0, 0 ) )
...
*
      BLOCK = ( ( NB.GT.1 ).AND.
     $    ( NB.GE.ILAENV( INBMIN, 'DGEQRF', ' ', M, N, 0, 0 ) ).AND.
     $    ( MN.GE.ILAENV( IXOVER, 'DGEQRF', ' ', M, N, 0, 0 ) ) )
*
...
*     ***************************************************
*
      CALL DGEQPC( JOB, M, N, K, A, LDA, C, LDC, 1, 0,
...
*
            ...
            CALL DGEQPW( M, LWSIZE, KB, KK-1, LACPTD, A, LDA,
...
*
               CALL DLARFT( 'Forward', 'Columnwise', M-KK+1,
...
*
     ...
     $                      MIN( KK+LWSIZE-1, STREJ-LWSIZE+LACPTD-1 )
                     MVIDX = MVIDX - 1
                     CALL DSWAP( M, A( 1, I ),1, A( 1, MVIDX ),1 )
                     ITEMP = JPVT( I )
                     JPVT( I ) = JPVT( MVIDX )
                     JPVT( MVIDX ) = ITEMP
 40               CONTINUE
...
*
            CALL DGEQPC( JOB, M, N, K, A, LDA, C, LDC, MN-KK+1, KK-1,
     $                   RCOND, LACPTD, JPVT, WORK( 1 ), WORK( MN+1 ),
     $                   SVLUES, MXNM, WORK( 2*MN+1 ), LWORK-2*MN )
            RANK = ACCPTD + LACPTD
         ELSE
...
*
         ACCPTD = 1
         CALL DGEQPC( JOB, M, N, K, A, LDA, C, LDC, MN-ACCPTD, ACCPTD,
     $                RCOND, LACPTD, JPVT, WORK( 1 ), WORK( MN+1 ),
     $                SVLUES, MXNM, WORK( 2*MN+1 ), LWORK-2*MN )
         RANK = ACCPTD+LACPTD
*
